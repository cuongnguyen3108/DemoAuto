...
public class LoginPage extends BasePage {
    ...
    public void login(String username, String password) {
        ...
        for (Map<String, String> data : excelData) {
            WebElement inputUsername = WaitElement.visible(driver,By.xpath(firstRow.get("element_username")), 10);
            ElementValidate.clearAndType(inputUsername,data.get(username));
            WebElement inputPassword = WaitElement.visible(driver, By.xpath(firstRow.get("element_password")), 10);
            ElementValidate.clearAndType(inputPassword,data.get(password));

            WebElement btnLogin = WaitElement.visible(driver, By.xpath(firstRow.get("element_btnLogin")), 10);
            btnLogin.click();

            String msg = ElementValidate.validate(driver, null, "Login", By.cssSelector(firstRow.get("element_msg")));
